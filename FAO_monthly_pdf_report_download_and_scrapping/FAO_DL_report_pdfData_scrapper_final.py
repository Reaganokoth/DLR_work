...
def FAO_DL_pdfReportData_scrapper(path):
    import pandas as pd
    import os
    import re
            ...
            while re.findall(forecast_pattern, string_to_search) == []:
                ...
                for y in range(len(treated_area_as_str)):
                    if y != len(treated_area_as_str) - 1:
                        treated_area_as_int.append(
                            int(re.sub(r"((treated|estimated)\s|,?)", "", treated_area_as_str[y][0])))
                    else:
                        treated_area_as_int.append(
                            int(re.sub(r"((treated|estimated)\s|,?)", "", treated_area_as_str[y][0])))
    ...
    df.to_csv(f"{path}2019_2021_DL_Treatment_data_test.csv", index=False)
    return df


FAO_DL_pdfReportData_scrapper(path=path)  # implementation 1
#my_data = FAO_DL_pdfReportData_scrapper(path=path)  # implementation 2
